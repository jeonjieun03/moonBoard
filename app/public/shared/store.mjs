// ALEPH T04 공식 참조 어댑터(adapter-reset.example.js)를 ESM으로 옮긴 상태 저장 엔진.
// 로직은 원본과 동일하게 유지하고(README: "그대로 복사해야 하는 라이브러리는 아니다" — 강제는 아니지만
// 검증 함수를 그대로 쓰는 편이 안전해서 채택), 화면 표시용 부가정보(phase_name 등)를 `display` 필드로만
// 얹을 수 있게 최소 확장했다. normalized-reading.schema.json의 9개 필드(reading)는 원본 그대로 건드리지 않는다.
//
// 이 모듈 하나를 실제 수집(collect.mjs)과 브라우저 합성 재생(fixtures 패널)이 함께 import해서 쓴다.
...
/** display: {phase_name, moonrise, moonset, raw} 같은 화면 표시용 부가정보 (reading 스키마 밖에 별도 보관). */
export function applySuccessfulReading(inputState, reading, runMeta = {}, display = null) {
  ...
  const row = {
    ...
    display: display ? clone(display) : existing?.display ?? null,
  };
...
/** 공식 fixture.schema.json 형태의 fixture 객체 하나를 재생한다. */
export function runFixture(inputState, fixture, display = null) {
  const meta = {
  ...
  if (fixture.transport.status >= 200 && fixture.transport.status < 300) {
    try {
      return applySuccessfulReading(inputState, fixture.payload, meta, display);
    } catch {
...
/** 두 일별 값으로 전일 대비 변화를 다시 계산한다 (저장된 원본 값 기준). */
export function recomputeDelta(earlier, later) {
  if (!earlier || !later) return null;
  return { from: earlier.record_date, to: later.record_date, delta: later.normalized_value - earlier.normalized_value, unit: later.unit };
}
